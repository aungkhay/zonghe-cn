  ...
  <div class="password-container">
    ...
    <div class="content">
      ...
      <div class="password-section">
        <div class="password-label">{{ $t("f_reset_withdrawal_password") }}</div>

        <!-- 新密码输入 -->
        <div class="password-input-section">
          <div class="pin-header">
            <div class="input-label">{{ $t("f_new_password") }}</div>
            <svg-icon :name="showNewPassword ? 'visible' : 'invisible'" class="text-[20px] theme-svg-color-dark" @click="toggleNewPasswordVisibility" />
          </div>
        ...
        <div class="password-input-section">
          <div class="pin-header">
            ...
            <svg-icon :name="showConfirmPassword ? 'visible' : 'invisible'" class="text-[20px] theme-svg-color-dark" @click="toggleConfirmPasswordVisibility" />
          </div>
          <div class="pin-input-container">
        ...
        <div class="input-verify"
          ...
          <span style="color: white;">{{ $t("f_password_mismatch_error") }}</span>
        </div>
      </div>
...
import { resetJyPassword } from "@/api/withdraw";
...
const handleConfirm = async () => {
  ...
  const { code, data }: any = await resetJyPassword({
...
.password-input-section {
  ...
  .pin-input-container {
    ...
    .pin-input {
      width: 60px;
      height: 55px;
      background-color: $theme-withdraw-pin-input-bg;
      display: flex;
      justify-content: center;
      align-items: center;
...
.notice-section {
  ...
  .notice-text {
    font-size: 14px;
    font-weight: bold;
    color: $theme-withdraw-action-color;
    line-height: 1.5;
    padding: 12px;
  }
}
