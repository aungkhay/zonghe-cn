<template>
  <div class="main-box">
     ...
     <div class="descript">
        ...
        <div class="descript-content">{{t("day_bet_descript_content")}} </div>
     </div>
...
import { ActiveDayBetConfigVo} from '@/model/activityEnum';
  ...
  const getTabData = async () => {
    const {code, data} = await getGlobalSetting<ActiveDayBetConfigVo>({name:"ACTIVE_DAY_BET"});
    console.log(data);
    if(code == 200){
  ...
  const getRewardData = async () => {
    const {code, data} = await getBetReward({type:0});
    if(code == 200){
  ...
  const handleClaim = async () => {   
    ...
    isClicking = true;
    const {code, data} = await receiveBetReward({type:0});
    if(code == 200){
       showTipModel({
        type: 4,
