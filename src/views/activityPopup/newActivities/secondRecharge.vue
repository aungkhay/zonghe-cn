  ...
  <div class="main-box">
     ...
     <div class="table-content">
         <div class="table-title-row">
            <div class="table-title-cell">{{t("second_recharge")}} </div>
     ...
     <div class="descript">
        ...
        <div class="descript-content">{{t("second_recharge_descript_content")}} </div>
     </div>
  </div>
</template>
...
import { ActiveSecondRechargeConfigVo } from '@/model/activityEnum';
import common from '@/utils/common/common';
  import { onMounted, ref } from 'vue';
  import { useI18n } from 'vue-i18n';
  const { t } = useI18n();

  const realData = ref<ActiveSecondRechargeConfigVo>()
  
  onMounted(async () => {
      const {code, data} = await getGlobalSetting<ActiveSecondRechargeConfigVo>({name:"ACTIVE_SECOND_RECHARGE"});
      if(code == 200){
