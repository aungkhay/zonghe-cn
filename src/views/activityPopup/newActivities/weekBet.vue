<template>
    <div class="main-box">
       ...
       <div class="descript">
          ...
          <div class="descript-content">{{t("week_bet_descript_content")}} </div>
       </div>
...
import common from '@/utils/common/common';
    ...
    const getTabData = async () => {
      const {code, data} = await getGlobalSetting<ActiveWeekBetConfigVo>({name:"ACTIVE_WEEK_BET"});
      console.log(data);
      if(code == 200){
    ...
    const getRewardData = async () => {
      const {code, data} = await getBetReward({type:1});
      if(code == 200){
    ...
    const handleClaim = async () => {   
      ...
      isClicking = true;
      const {code, data} = await receiveBetReward({type:1});
      if(code == 200){
         showTipModel({
          type: 4,
