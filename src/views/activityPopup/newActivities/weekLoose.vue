  ...
  <div class="main-box">
     ...
     <div class="table-content">
         <div class="table-title-row">
            <div class="table-title-cell">{{t("loss")}} </div>
     ...
     <div class="descript">
        ...
        <div class="descript-content">{{t("week_loose_descript_content")}} </div>
     </div>
...
<script lang="ts" setup>
  import { getBetReward, getGlobalSetting, receiveBetReward } from '@/api/activityPopup';
  import { showTipModel } from '@/components/TipModel';
  import { ActiveDayBetConfigVo, ActiveWeekBetConfigVo} from '@/model/activityEnum';
  ...
  const getTabData = async () => {
    const {code, data} = await getGlobalSetting<ActiveWeekBetConfigVo>({name:"ACTIVE_WEEK_LOOSE"});
    console.log(data);
    if(code == 200){
  ...
  const getRewardData = async () => {
    const {code, data} = await getBetReward({type:2});
    if(code == 200){
  ...
  const handleClaim = async () => {   
    ...
    isClicking = true
    const {code, data} = await receiveBetReward({type:2});
    if(code == 200){
       showTipModel({
        type: 4,
