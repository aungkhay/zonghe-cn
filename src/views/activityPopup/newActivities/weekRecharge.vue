<template>
    <div class="main-box">
       ...
       <div class="descript">
          ...
          <div class="descript-content">{{t("week_recharge_descript_content")}} </div>
       </div>
...
import common from '@/utils/common/common';
    ...
    const handleClaim = async () => {   
      ...
      isClicking = true
      const {code, data} = await receiveRechargeReward({type: 1});
      if(code == 200){
         showTipModel({
          type: 4,
    ...
    const getTabData = async () => {
        const {code, data} = await getGlobalSetting<ActiveWeekRechargeConfigVo>({name:"ACTIVE_WEEK_RECHARGE"});
        if(code == 200){
    ...
    const getRewardData = async () => {
        const {code, data} = await getRechargeReward({type: 1});
        console.log(data);
        if(code == 200){
