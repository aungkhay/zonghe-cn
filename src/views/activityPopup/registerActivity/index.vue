            ...
            <div class="light flex items-center justify-center">
                <div class="bg flex flex-col items-center justify-center">
                    ...
                    <div v-scaleTap class="button flex items-center justify-center mt-[16px]" :class="{'not-active': !isCanGet}" @click="onClickButton">
                        <svg-icon class="text-[18px] mr-[8px] theme-svg-color-light" name="gift" />
                        <div class="ml-[8px]">   
                            {{ $t("104002") }}
                        </div>
                    </div>  
...
import { getGlobalSetting, getRegisterAndDownloadReward, receiveRegisterReward } from '@/api/activityPopup';
...
const popupShow = computed(()=>{
    if(useNewActPopupStoreHook().popups.registerActivity.isShowData){
        getTabData();
        getRewardData();
        useNewActPopupStoreHook().popups.registerActivity.isShowData = false;
    }
    return useNewActPopupStoreHook().popups.registerActivity.isShow;
})
...
const {t} = useI18n();
const closePopup = () => {
    useNewActPopupStoreHook().closePopup("registerActivity");
}
const reward = ref<number>(0);
const onClickButton = async() => {
    if(isCanGet.value){
        // 领取
        const {code, data} = await receiveRegisterReward();
        if(code == 200){
            showTipModel({
                type: 4,
...
const getTabData = async () => {
    const {code, data}:any = await getGlobalSetting({name:"ACTIVE_REGISTER"});
    // console.log(data);  
    if(code == 200){
...
const getRewardData = async () => {
    ...
    if(code == 200){
        isCanGet.value = data.data.registerAmount > 0;
    }
}
...
<style lang="scss" scoped>
    ...
    .title {
        width: 168px;
        height:85.6px;
        background:url('#{$img-base-path}/event/newMarquee/title_2.png');
        background-size: 100% 100%;
        background-repeat: no-repeat;
        background-position: center center;
    }
